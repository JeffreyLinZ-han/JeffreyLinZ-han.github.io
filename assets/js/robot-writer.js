...
class RobotWriterWeb{
 ...
 async animateQ(q0,q1,dur,token){const start=performance.now();return new Promise(resolve=>{const step=(now)=>{if(token!==this.cancelToken)return resolve(false);let u=M.clamp((now-start)/(dur*1000),0,1),s=smooth(u);for(let i=0;i<5;i++)this.v.values[i]=q0[i]+(q1[i]-q0[i])*s;this.v.syncControls();this.v.requestRender();if(u<1)requestAnimationFrame(step);else resolve(true);};requestAnimationFrame(step);});}
